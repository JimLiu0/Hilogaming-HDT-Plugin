    ...
    public class BattlegroundsGameCollection : IDisposable
    {
        ...
        private async Task OnGameStart()
        {
            if (Core.Game.CurrentGameMode == GameMode.Battlegrounds)
            {
                ...
                // Get starting MMR from BattlegroundsLastGames
                var lastGames = await BattlegroundsLastGames.Instance.PlayerGames(false);
                if (lastGames != null && lastGames.Any())
                {
                    _startingMmr = lastGames.First().Rating;
                    Hearthstone_Deck_Tracker.Utility.Logging.Log.Info($"OnGameStart - Set starting MMR to: {_startingMmr} from BattlegroundsLastGames");
                }
